...
ScheduleArg = (
    ScheduleInterval | Timetable | BaseAsset | Collection[Union["Asset", "AssetAlias"]]
)


def infer_automated_data_interval(
    timetable: Timetable, logical_date: datetime
) -> DataInterval:
    """
...
def get_run_data_interval(timetable: Timetable, run: DagRun) -> DataInterval:
    ...
    if (
        data_interval := timetable.infer_manual_data_interval(
            run_after=run.logical_date
        )
    ) is not None:
        return data_interval
...
def get_next_data_interval(
    timetable: Timetable, dag_model: DagModel
) -> DataInterval | None:
    """
...
class DagModel(Base):
    ...
    def deadline(self):
        ...
        if isinstance(self._deadline, list):
            return [
                DeadlineAlert.deserialize_deadline_alert(item)
                for item in self._deadline
            ]
    ...
    def deadline(self, value):
        ...
        if value is None:
            ...
            self._deadline = [
                item if isinstance(item, dict) else item.serialize_deadline_alert()
                for item in value
            ]
